...
if (!function_exists('uploadAppImage')) {
    function uploadAppImage($image, $name, $module = "images")
    {
        ...
        $original = 'images/' . $module . '/Original/';
        ...
        $fileName = $original . Carbon::now('America/Fortaleza')->format('YmdHis') . '_' .
            Str::random(4) . '_' .
            rand(10, 99) . rand(10, 99) . rand(10, 99) . '_' .
            Str::random(4) . '.' .
            $ext;
        ...
        $img = Image::make($filePath);
        $img->resize(config('upload.' . $module . '.width'), config('upload.' . $module . '.height'), function ($constraint) {
            $constraint->aspectRatio();
            $constraint->upsize();
        });
